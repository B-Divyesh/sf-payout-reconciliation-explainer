...
function renderSourceEvidence(): string {
  ...
  return kinds.map((kind) => {
    const dataset = state.datasets[kind]!;
    return `<section class="source-group" aria-labelledby="source-${kind}-title"><h4 id="source-${kind}-title">${kindLabels[kind].title}</h4><div class="source-table-wrap"><table class="source-table"><caption>${escapeHtml(dataset.fileName)} · ${records[kind].length} rows</caption><thead><tr><th scope="col">Source row</th><th scope="col">Mapped ID</th><th scope="col">Mapped date</th><th scope="col">Mapped amount</th><th scope="col">Original values</th></tr></thead><tbody>${records[kind].map((row) => `<tr><td data-label="Source row">${row.row}</td><td data-label="Mapped ID">${escapeHtml(row.id)}</td><td data-label="Mapped date">${escapeHtml(row.date)}</td><td data-label="Mapped amount" class="money">${minorToDecimal(row.amountMinor, state.result!.decimals)} ${state.result!.currency}</td><td data-label="Original values"><dl class="original-values">${Object.entries(row.original).map(([header, value]) => `<div><dt>${escapeHtml(header)}</dt><dd>${escapeHtml(value)}</dd></div>`).join('')}</dl></td></tr>`).join('')}</tbody></table></div></section>`;
  }).join('');
}
